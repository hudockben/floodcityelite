...
export default function PaymentSearch({
  ...
  function pick(match: PlayerMatch) {
    onPick(match);
    // Reset so the field is ready for the next payment.
    setQuery("");
    setOpen(false);
  }
    ...
    <div className="pay-search" ref={rootRef}>
      <div className="pay-search-field">
        ...
        <input
          ...
          placeholder="Search a player or team to add a payment…"
          value={query}
          role="combobox"
          aria-expanded={showDropdown}
          aria-controls="pay-search-list"
          aria-autocomplete="list"
          aria-activedescendant={
            showDropdown && results.length > 0
              ? `pay-search-opt-${active}`
              : undefined
          }
          autoComplete="off"
          onChange={(e) => {
      ...
      {showDropdown ? (
        <ul className="pay-search-list" id="pay-search-list" role="listbox">
          {results.length === 0 ? (
